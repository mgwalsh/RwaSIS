...
wheat <- as.data.frame.matrix(table(cro$survey, crop$wheat > 0))
...
The `anova` results in the chunk above, the SAEs reported by `GID` also produce significantly more precise estimates than those produced by reporting by `sectors`. This may be an artifact of the current `bean` distribution survey data available that were used in this particular SAE model comparison, which bears further investigation. Nonetheless, surveys of new geographical areas or for monitoring and/or planning and experimentation would be well informed to keep the statistical procedures provided by this notebook in mind. The table of observations and the relevant SAE `bean` estimates by `GID` are generated in the chunk below. These can be consulted for planning additional surveys and/or experiments.
