...
* This notebook produces precise, accurate and reproducible spatial predictions of the observed occurrence (presence / absence) of bean occurrence data in Rwanda based on 4 FAIRly (see [FAIR](https://en.wikipedia.org/wiki/FAIR_data)) distributed crop/soil surveys and using a standard ensemble machine learning workflow.
...
* We recommend a standard small equal area sampling frame (such as the `GID`) over admistrative regions for SAEs. The reasons for this are both operational and conceptual. Operationally, `GID` can be navigated to easily just by using a GPS. Conceptionally, `GID`, unlike administrative units, will not change over time in Rwanda ... barring any major meteor impacts.

* There are other notable, important and prevalent small-holder crops that probably warrant attention in Rwanda's R&D agenda (e.g., Sorghum, Banana and vegetables), which are currently not priorities for surveys and/or management response trials, at least within RwaSIS.
