...
The table of observations and the relevant SAE `bean` estimates by `GID` are generated in the chunk below based on the `m2` SAE model results in the combined survey. These should be consulted and updated for planning additional survey activities and/or experiments that require crop distribution and geographical guidance data. Again, the estimates can be flexibly extended to include other crop distributions.
...
* This notebook produces precise, accurate and reproducible spatial predictions of the observed occurrence (presence / absence) of bean occurrence data in Rwanda based on a standard ensemble machine learning workflow.
