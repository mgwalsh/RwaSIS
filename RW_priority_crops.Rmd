...
# Write out file
coordinates(gidxyxy) <- ~x+y
crs(gidxy) <- "+proj=laea +ellps=WGS84 +lon_0=20 +lat_0=5 +units=m +no_defs"
gidll <- spTransform(gidxy, CRS(proj4string(shape)))
colnames(gidll) <- c("lon","lat")
gidxy <- cbind(gidxy, gidll)
...
gsIndex <- createDataPartition(gsdat$potato, p = 4/5, list = F, times = 1)
