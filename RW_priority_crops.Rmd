...
```{r}
# Package names
packages <- c("downloader", "rgdal", "sp", "raster", "leaflet", "DT", "htmlwidgets", "devtools", "caret", "caretEnsemble", "mgcv", "MASS", "randomForest", "xgboost", "klaR", "nnet", "plyr", "dplyr", "doParallel", "dismo", "arm")
...
Note that the defined `GID` variable in the chunk above is sort of like a military style 10 × 10 km grid ID that will usually contain several survey locations. You can use as waypoint input to a GPS (see e.g: [GPSBabel](https://www.gpsbabel.org)), tablet or smart phone for in-the-field navigation. We will also use it further on in the script for SAE as an alternative to using potentially problematic administrative units. 
...
Note that while we are using the `bean` distribution data here, you can also substitute the other 5 crops as labels and specify those with the `labs` variable in the chunk above. The 5 additional crop labels are included in the `crop` dataframe, but we shall leave those for you to explore. Presented next are some starter models, which we consider to be *contrasting* both in terms of the gridded calibration features that are used, as well as the MLAs that are used in fitting the observational data to those.
...
This next chunk fits a model ensemble with the `glmStepAIC` function from the `MASS` library using the *validation dataframe* (`gs_val`). You could explore other [meta-model](https://machinelearningmastery.com/meta-learning-in-machine-learning/) options here, but we think that this approach provides a reasonable combination and weighting of the 6 models that were produced in the previous training steps. Again, the fitting is done with cross-validation.
...
The next chunk sets writes out the 8 prediction grids to a geotif file, which can be imported to a GIS of your choice. It also generates an overview map of the cropland occurrence probabilities for `beans` in this particular example.
...
m0 <- glmer(bean ~ 1 + (1|sector), family = binomial, gsout) ## base model
m1 <- glmer(bean ~ st + (1|sector), family = binomial, gsout)
```
...
m2 <- glmer(bean ~ st + (1|GID), family = binomial, gsout)
anova(m0, m1, m2)
```
...
* This notebook produces precise, accurate and reproducible spatial predictions of the observed presence/absence of bean occurrence data in Rwanda based on 4 *FAIRly* (see [FAIR](https://en.wikipedia.org/wiki/FAIR_data)) distributed crop/soil surveys and using a standard ensemble machine learning workflow.

* The predictions can be flexibly extended to include other priority crop or cropping system distributions and small area estimates that are needed for planning of management response trials and monitoring activities.
...
* There are other notable and prevalent smallholder crops that probably warrant attention in Rwanda's R&D agenda (e.g., Sorghum, Banana and vegetables), which are currently not priorities for surveys and/or management response trials, at least within RwaSIS.
