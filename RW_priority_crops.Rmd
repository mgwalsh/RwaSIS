...
2. The [CIALCA Farm Heterogeneity Survey of Rwanda, (2018)](https://cialca.shinyapps.io/cialca_base_2files/) survey desinated as `other`. There are 3,199 georeferenced observations covering Rwanda in this survey. Note that the spatial coordinates for this survey are slightly offset to the GPS locations of the surveyed households rather than to the actual locations of fields.
...
# Render map
w <- leaflet() %>% 
  setView(lng = mean(gsdat$lon), lat = mean(gsdat$lat), zoom = 8.5) %>%
  addProviderTiles(providers$OpenStreetMap.Mapnik) %>%
...
The data table of observations and the relevant SAE `bean` estimates by `GID` are generated in the chunk below based on the `m2` SAE model results in the combined survey. These should be consulted and updated for planning additional survey activities and/or experiments that require crop distribution and geographical guidance data. They are written out to your `./Priority_crops/Results` directory. Again, the estimates can be flexibly extended to include other crop distributions.
...
* This notebook produces precise, accurate and reproducible spatial predictions of the observed occurrence (presence / absence) of bean occurrence data in Rwanda based on 4 fairly crop/soil surveys and using a standard ensemble machine learning workflow.

* The predictions can be flexibly extended to include other priority crop distribution predictions and small area estimates that are needed for planning of response trials and monitoring activities.

* We recommend a standard small equal area (such as the `GID`) over admistrative regions for SAEs. The reasons for this are both operational and conceptual. Operationally, `GID` can be navigated to easily just by using a GPS. Conceptionally, `GID`, unlike administrative units, will not change over time ... barring any meteor impacts.
