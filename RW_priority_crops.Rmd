...
Note that based on the `anova` results, the SAEs reported by `GID` also produce significantly more precise estimates than those that were produced by reporting by `sectors`. This may be an artifact of the current `bean` distribution data that were used in this comparison, which bears further investigations.
