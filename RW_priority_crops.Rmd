...
# Change dependent variable labels from `bean` to `maize`, `cassava` ... here
...
Note that based on the `anova` results in the chunk above, the SAEs reported by `GID` also produce significantly more precise estimates than those produced by reporting by `sectors`. This may be an artifact of the current `bean` distribution survey data available that were used in this particular SAE model comparison, which bear further investigations. Nonetheless, surveys of new geographical areas or for monitoring and/or planning and experimentation would be well informed to keep the statistical procedures provided by this notebook in mind.

```{r, echo = FALSE}
summary(m2)
```

Generate table ... of observations and prediction by GID
