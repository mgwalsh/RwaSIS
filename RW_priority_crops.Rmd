...
Note that the defined `GID` variable in the chunk above is sort of like a military style 10 × 10 km grid ID that will usually contain several survey locations. You can use as waypoint input to a GPS (see e.g: [GPSBabel](https://www.gpsbabel.org)), tablet or smart phone for in-the-field navigation. We will also use it further on in the script for SAE as an alternative to using the administrative units. 
...
Note that based on the `anova` results in the chunk above, the SAEs reported by `GID` also produce significantly more precise estimates than those produced by reporting by `sectors`. This may be an artifact of the current `bean` distribution survey data available that were used in this particular SAE model comparison, which bears further investigation. Nonetheless, surveys of new geographical areas or for monitoring and/or planning and experimentation would be well informed to keep the statistical procedures provided by this notebook in mind. The table of observations and the relevant SAE `bean` estimates by `GID` are generated in the chunk below. These can be consulted for planning additional surveys or experiments.
