...
The `anova` results in the chunk above, the SAEs reported by `GID` produce significantly more precise estimates than those produced by reporting by `sectors` and relative to the `m0` base model, which does not take the spatial predictions into account. This may be an artifact of the current `bean` distribution survey data that were available and used in this particular SAE model comparison, which bears further investigation. Nonetheless, surveys of new geographical areas or for monitoring and/or planning and experimentation would be well informed to keep the statistical procedures provided by this notebook in mind. Also see the `m2` SAE model estimates below.

```{r, echo = FALSE}
summary(m2)
``` 

The table of observations and the relevant SAE `bean` estimates by `GID` are generated in the chunk below based on the `m2`. These should be consulted for planning additional surveyactivities and/or experiments that require crop distribution data.
