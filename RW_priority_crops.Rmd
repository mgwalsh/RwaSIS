...
This next chunk reprojects the crop data to the [Lambert Azimuthal Equal Area (LAEA)](https://en.wikipedia.org/wiki/Lambert_azimuthal_equal-area_projection) grid of the AfSIS raster variables and then writes out the combined dataframe `RW_priority_crop_dist.csv` into your `./Priority_crops/Results` directory, if you'd like to process those outputs in software other than R. It also generates a location map of where in Rwanda those 7.5k+ crop survey labels were obtained.
...
# Write out the file
write.csv(sae, "./Priority_crops/Results/sae_dist_by_GID.csv", row.names = F)
```
