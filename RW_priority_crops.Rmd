---
title: Crop distribution predictions from survey data and small area estimates in Rwanda
...
*The main intent of this notebook is to demonstrate starter code for predictive crop species distribution mapping and the associated statistical small area estimates ([SAE](https://www.census.gov/srd/csrm/SmallArea.html)), which define the crop species distributions in a given (sub)region of interest ([ROI](https://en.wikipedia.org/wiki/Region_of_interest)).* 

We use both Rwanda's most recent and some of its legacy cropland survey (label) data here and the associated raster features to illustrate the general approach and the main data analysis steps. Rwanda, being a small country, is convenient for this illustration because the script will run fairly fast so as to not test your patience in running it. The notebook is publicly maintained on [Github](https://github.com/mgwalsh/RwaSIS/blob/master/RW_priority_crops.Rmd), and you can fork and modify it from there as you see fit. You may also want to take a look at the [GeoSurvey cropland area notebook](https://osf.io/shkxp/), which describes some of the spatial prediction methods and the associated SAE estimation procedures in more detail.
...
```{r}
# Package names
packages <- c("osfr", "downloader", "rgdal", "sp", "raster", "leaflet", "DT", "htmlwidgets", "devtools", "caret", "caretEnsemble", "mgcv", "MASS", "randomForest", "xgboost", "klaR", "nnet", "plyr", "dplyr", "doParallel", "arm")
...
# Download crop data
osf_retrieve_file("zqh9y") %>% osf_download(conflicts = "overwrite")
...
osf_retrieve_file("hp6v7") %>% osf_download(conflicts = "overwrite")
unzip("RW_250m_2020.zip", overwrite = T)
osf_retrieve_file("u73pd") %>% osf_download(conflicts = "overwrite")
unzip("RW_GS_preds.zip", overwrite = T)
glist <- list.files(pattern="tif")
grids <- stack(glist)
```
...
The `scode` survey designations of either `base` or `other` are important for adjusting the SAEs that are calculated as fixed effects relative to the currently ongoing, spatially balanced RwaSIS cropland survey. Note that because we cannot replicate the assumptions regarding the sampling frames for the `scode = other` surveys here, we have to debias them in the SAEs that are generated by this workflow.
...
The pre-processed Rwanda grid data (in the `grids` raster stack) were derived and reprojected from their primary open sources. You can also download the entire stack directly at [RwaSIS grids](https://osf.io/hp6v7/). The short descriptions of the included rasters, and their sources are provided in the table below.
...
This next chunk reprojects the crop data to the [Lambert Azimuthal Equal Area (LAEA)](https://en.wikipedia.org/wiki/Lambert_azimuthal_equal-area_projection) grid of the AfSIS raster variables and then writes out the combined dataframe `RW_priority_crop_dist.csv` into your `./Priority_crops/Results` directory, if you'd like to process those outputs in software other than R. It also generates a location map of where in Rwanda those 7k+ crop survey labels were obtained.
...
# Write out number of survey observations by GID and survey code
gidsc <- table(gsdat$GID, gsdat$scode)
gidsc <- as.data.frame.matrix(gidsc)
write.csv(gidsc, "./Priority_crops/Results/GID_scode_table.csv", row.names = T)
...
Note that the defined `GID` variable in the chunk above is like a military style 10 × 10 km grid ID that will usually contain several survey labels. You can use as waypoint input to a GPS (see e.g., [GPSBabel](https://www.gpsbabel.org)), tablet or smart phone for in-the-field navigation. We will also use it further on in the script for SAE as an alternative to using potentially problematic administrative units. The next chunk writes out the number of the current survey labels per `GID` as well as the center coordinates of those. You could also try something similar with the [`dggridR` package](https://www.rdocumentation.org/packages/dggridR/versions/2.0.4), which implements several kinds of discreet global grids.

```{r}
# Extract current survey GID coordinates
gx <- ceiling(gsdat$x/res.pixel)*res.pixel - 0.5*res.pixel ## center x coordinate of GID
gy <- ceiling(gsdat$y/res.pixel)*res.pixel - 0.5*res.pixel ## center y coordinate of GID

# Assemble dataframe
xgid <- ceiling(abs(gx)/res.pixel)
ygid <- ceiling(abs(gy)/res.pixel)
gidx <- ifelse(gx < 0, paste("W", xgid, sep=""), paste("E", xgid, sep=""))
gidy <- ifelse(gy < 0, paste("S", ygid, sep=""), paste("N", ygid, sep=""))
GID <- paste(gidx, gidy, sep="")
GID <- as.data.frame(cbind(GID, gx, gy))
GID$gx <- as.numeric(as.character(GID$gx))
GID$gy <- as.numeric(as.character(GID$gy))

# Calculate total number of current survey labels (n) per GID, and their center coordinates
gidxy <- ddply(GID, c("GID"), summarise,
               n = length(GID),
               x = mean(gx),
               y = mean(gy))

# Write out file
write.csv(gidxy, "./Priority_crops/Results/GID_center_points.csv", row.names = F)
coordinates(gidxy) <- ~x+y
crs(gidxy) <- "+proj=laea +ellps=WGS84 +lon_0=20 +lat_0=5 +units=m +no_defs"
gidll <- as.data.frame(spTransform(gidxy, CRS(proj4string(shape))))
colnames(gidll) <- c("GID", "n", "lon", "lat")
write.csv(gidll, "./Priority_crops/Results/GID_center_points_ll.csv", row.names = F)
```
...
Central places (*sensu* [Central place theory](https://en.wikipedia.org/wiki/Central_place_theory)) are influential variables for predicting of where specific crops are likely to occur (or not, e.g., in city centers, forest reserves or national parks). The model below focuses on central place variables such as distances to major and minor roads, urban & rural settlements, parks & reserves, cell towers & electricity networks among other largely anthropically controlled or infrastructure variables. 
...
The next chunk writes out the 8 prediction grids to a geotif file `./Priority_crops/Results/"RW_bean_preds_2020.tif`, which can be imported to a GIS of your choice. It also generates an overview map of the cropland occurrence probabilities for `beans` in this particular example.
...
# Apply cropland mask
cpmask <- raster("./Priority_crops/CM20.tif")
...
```{r, warning = FALSE}
# Small area hierarchical random-effect estimates by GIDs
m2 <- glmer(potato ~ scode*st + (1|GID), family = binomial, gsout)
anova(m0, m1, m2)
```
...
The data table of observations and the relevant SAE `bean` estimates by `GID` are generated by the chunk below based on the `m2` SAE model results for the combined survey. These should be consulted and updated for planning additional survey activities and/or experiments that require crop distribution and geographical guidance data. They are written out to your `./Priority_crops/Results` directory. Again, the estimates can be flexibly extended to include other crop distributions, and the current estimates for all 6 priority crops are provided at the [RwaSIS OSF repository](https://osf.io/k8y3z/) for your consideration. 

```{r}
# Generate cross-tabulation of the observations
sae <- as.data.frame.matrix(table(gsout$GID, gsout$bean > 0)) ## substitute other priority crop names here
sae$GID = rownames(sae)

# Generate SAE model estimates and merge by GID
sae.ran <- ranef(m2)
sae.rse <- se.ranef(m2)
...
sae <- merge(sae, sae.est, by = "GID")
colnames(sae) <- c("GID", "absent", "present", "ran", "se")

# Write out the file
write.csv(sae, "./Priority_crops/Results/potato_dist_by_GID.csv", row.names = F)
```
...
* This notebook produces precise, accurate and reproducible spatial predictions of the observed presence/absence of bean occurrence data in Rwanda based on 4 [FAIR](https://en.wikipedia.org/wiki/FAIR_data) crop/soil surveys and using a standard ensemble machine learning workflow. The predictions can be flexibly extended to include other priority crop or cropping system distributions and small area estimates that are needed for planning of management response trials and/or monitoring activities.
