...
xcen <- ceiling(gsdat$x/res.pixel)*res.pixel ## center x coordinate of GID
ycen <- ceiling(gsdat$y/res.pixel)*res.pixel ## center y coordinate of GID
...
gsdat <- cbind(GID, xcen, ycen, gsdat)
...
```{r, echo = FALSE, results = 'hide'}
# Assemble existing survey GID coordinates
gx <- gsdat %>% dplyr::group_by(GID) %>% summarise(mean(xcen))
gy <- gsdat %>% dplyr::group_by(GID) %>% summarise(mean(ycen)-res.pixel)
gidxy <- merge(gx, gy, by = "GID")
colnames(gidxy) <- c("GID", "x", "y")
