...
Model `m1` is preferable to the overly simple `m0` model based on the respective AIC, BIC and logLik comparisons. The `m2` model, which substitutes the `lime amount` that was used for if `lime` was used.   
...
* In the two studies presented where the application of lime did not have large effects on either bean or maize crop control yields (i.e., no lime application) that were observed over a two year followup period.
