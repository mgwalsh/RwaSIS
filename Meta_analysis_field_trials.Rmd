...
It is common practice in meta-analysis that the tables of e.g., a [Cochrane review article](https://www.cochranelibrary.com/) summarize the descriptive statistics of the characteristics of the study populations, stratified by the main exposure/treatment variables by trials. The [`aggregate`](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/aggregate) function in the R [`stats`](https://www.rdocumentation.org/packages/stats/versions/3.6.2) package is convenient in this context. We use it here to shape the intervention and the varying effects into a consistent PICO data frame. You could also use the [`dplyr`](https://www.rdocumentation.org/packages/dplyr/versions/0.7.8) package for this.
...
plot(ecdf(m0$yield), main="", xlab="Maize yield (kg/ha)", ylab="Cum. proportion of observations", 
     xlim=c(0, 10000), verticals=T, lty=1, lwd=2, cex.lab=1.3, col="dark grey", do.points=F)
...
cont <- subset(pico, lime=="n",  select=c(site, season, crop, yield)) 

c0 <- lmer(yield ~ factor(crop)-1 + (1|season) + (1|site),
           data = cont)

c1 <- lmer(yield ~ factor(crop)-1 + (1|season) + (1|site) + (1|season:site),
           data = cont)
...
We can also plot the model fit to the trial data with the following chunk. This demonstrates that a fairly simple MLM can explain a substantial amount of the variation in a fairly complex dataset. However, also note that this plot is not intended as a posterior prediction of what out-of-sample trials might look like. It just shows an unvalidated fit to the data.
...
Other multilevel model specifications and analyses (e.g., see [Multilevel regression case studies](https://bookdown.org/jl5522/MRP-case-studies/)) in this context are certainly possible e.g., by also including variables such as crop varieties and genotype × environment interactions ([GE](https://en.wikipedia.org/wiki/Gene%E2%80%93environment_interaction)), seasonality, spot vs broadcast application, etc. But, the data would need to be collected and assembled to better reflect their specific PICO-differences and very importantly also the spatial variation for the more contingent factors of yield outcomes that were considered here.

## Results for the Rwanda trials

The main reason for fitting the MLMs that were used here has to do with borrowing information from other, similar trials ...

```{r, fig.align = "center", expected_yield, fig.show='hold', out.width="50%", fig.cap = "Cumulative distributions of expected maize and bean yields. In grey the control yields (no lime and no fertilizers), in blue, the addition of DAP, Urea and/or CAN fertilizers, in red, the addition of both lime and fertilizers."}

# Select Rwanda
rwa <- subset(pico, country=="RWA", select=c(lime, fertilizer, crop, yield, yhat))

# Maize yields
m0 <- subset(rwa, lime=="n" & fertilizer=="n" & crop=="maize", select=c(yhat)) 
m2 <- subset(rwa, lime=="n" & fertilizer=="y" & crop=="maize", select=c(yhat)) 
m3 <- subset(rwa, lime=="y" & fertilizer=="y" & crop=="maize", select=c(yhat)) 
...
# Bean yields
b0 <- subset(rwa, lime=="n" & fertilizer=="n" & crop=="beans", select=c(yhat)) 
b2 <- subset(rwa, lime=="n" & fertilizer=="y" & crop=="beans", select=c(yhat)) 
b3 <- subset(rwa, lime=="y" & fertilizer=="y" & crop=="beans", select=c(yhat)) 
...
* In the two trials presented the application of lime showed modest positive intervention effects on bean crop yields that were observed over a two year period vis-à-vis their respective control yields. The fairly low lime application rates in these trials may not have been sufficient to overcome the exchangeable (reserve) acidity constraints nor to effectively alter soil pH over the relatively short 2-year duration of the trials.

* As might be expected, space-time-crop interactions, e.g., the `cell:trial_age` and `cell:crop` random effects are likely to dominate production function based estimation and inference. These warrant more systematic investigations. One way of doing that would be to provide the georeference of each individual trial. A reanalysis of the data that are presented here, could be done quickly. 

* The simple initial question posed: "By how much could maize and bean yields be increased with lime applications?", could not be satisfactorily answered by the data presented in this notebook. We can only infer that there were modest yield improvements in some of the surveyed administrative units. What we do not know is if these local improvements warrant the associated lime application and environmental costs to farmers, in those places, over the long-term.
...
* East Africa is certainly not Nebraska, South Africa or Brazil. *Polycropping* or *intercropping* with local staple food varieties occurs virtually everywhere in Rwanda's smallholder production crop systems and elsewhere in East Africa (see: [Mixed Farming Systems](https://osf.io/7rt6c/)). Our recommendation is to start conducting agricultural input trials in ways that actually reflect those facts rather than on the basis of largely unrealistic *monocropping* practices.
