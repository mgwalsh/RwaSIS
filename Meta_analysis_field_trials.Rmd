---
title: Meta-analyses of agricultual lime application effects on crop yields from field survey data
...
Many surveys, such as this one, will have difficulties in obtaining representative samples of individual trials across geographical areas of interest. However, with proper statistical adjustment, non-representative surveys can be used to generate and monitor crop yields more accurately. While we cannot build response models for individual trials because those were not identified (georeferenced) or tracked over time, we will run some initial multilevel models grouped by `cell` and `trial_age` using the [`lme4`](https://cran.r-project.org/web//packages/lme4/lme4.pdf) package on the main PICO variables for the 2 studies.
...
m1 <- lmer(log(yield) ~ factor(trial_age) + factor(crop) * factor(lime) * factor(fert) + (1|study) +
          (1|cell:trial_age), data = lime)
m2 <- lmer(log(yield) ~ factor(trial_age) + factor(crop) * factor(lime_amt) * factor(fert) + (1|study) +
          (1|cell:trial_age), data = lime)
anova(m0, m1, m2)
```

Model `m1` is clearly preferable to the base (but overly simple) `m0` model based on the respective AIC, BIC and logLik comparisons in the model `anova`. Subsequently, the `m2` model, which substitutes the approximate `lime amount` that was used for if (or not) `lime` was used  provides a slightly better fit to the data. However, the choice between the `m1` and `m2` models is a matter preference. Because of the overall meta-analysis study design and for reasons of parsimony, we prefer model `m1`. The parameter estimates for `m1` are shown below.   

```{r}
display(m1)
```

Other multilevel model analyses in this context are certainly possible e.g., by also including variables such as crop varieties and genotype × environment interactions ([GE](https://en.wikipedia.org/wiki/Gene%E2%80%93environment_interaction)), seasonality, spot vs broadcast application, etc., but the data would need to be assembled to better reflect treatment and importantly spatial variation in crop yields. 

# Takeaways

The main takeaways of this notebook are the following:

* In the two studies presented the application of lime had only modest positive effects on either bean or maize crop yields that were observed over a two year followup period vis-à-vis their respective control yields.

* The simple question: "By how much aglime applications can improve maize and bean crop yields on the acid soils in Rwanda?", is not (cannot be) answered in this notebook. At present we can infer at present is that there are modest improvements in some of of the surveyed cells. What we do not know is if these local improvements warrant the lime application costs to farmers.
...
* East Africa is certainly not Nebraska or Brazil. Intercropping occurs virtually everywhere in Rwanda's smallholder production systems and elsewhere in East Africa. Our recommendation is to start conducting agricultural input trials in ways that actually reflect those facts rather than on the basis of monocropping practices.
