...
However, because of the lack georeferencing of the individual trials it is not possible to tell which trials were dropped-out during the second year in each study. Indeed, new trials may also have come into the studies during the follow-up period, which were not recorded as such ... but we have no way of telling. This might also have affected some of the counter-intuitive crop yield differences that were recorded over time.

```{r}
tri <- aggregate(lime$yield, list(lime$trial_age, lime$study, lime$crop), FUN = quantile, probs = c(0.05, 0.5, 0.95))
colnames(tri) <- c("trial_age", "study", "crop", "Q")
tri
```
...
* Specify lime provenance data (see e.g., [Rwanda aglime sources]()). Lime sources in Rwanda and elsewhere are quite variable in terms of their calcium cabonate equivalents (CCE), Ca/Mg ratios and estimated extractable limestone source reserves.
