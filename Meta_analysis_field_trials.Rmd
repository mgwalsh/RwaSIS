...
Because of the lack of georeferencing of the individual trials it is not possible to tell which trials dropped-out during the second year in each study. Also, new trials may have come into the studies during the follow-up period that were not recorded as such ... but we have no way of telling. This may have affected the somewhat counter-intuitive crop yield differences that were recorded by time-period in the 2 studies.

The simplest, shortest and fasted way of screening extreme value outliers from these data is to consider the yield percentiles by crop. We can initially examine this by using [quantile regression]() as follows in the next chunk to subsequently decide which (if any), of the extreme values to screen from further analyses.

```{r}
out.rq <- rq(yield ~ factor(crop) - 1, tau = c(0.01, 0.025, 0.5, 0.975, 0.99), data = lime)
out.val <- as.data.frame(predict(out.rq, lime))

# you can always cbind the out.val dataframe for further processing with e.g., ...
# lime <- cbind(lime, out.val)

out.rq
```

The other convenient thing about  is that you could also other PICO factors in the respective regressions for more comprehensive outlier / extreme values explorations. We shall leave those for you to test. 
