...
colnames(trial) <- c("trial_id","country", "season", "site","year","crop","lime_amt","treated_yield","control_yield")
trial$delta <- trial$treated_yield - trial$control_yield
```

We then aggregate by `season`, `site` and `crop` type to set up the PICO table for meta-analyses. Note that this can be done in a number of different ways that could include additional moderator variables and/or grouping factors.
...
```{r, fig.align = 'center', fig.show='hold', fig.cap = "Figure 1: Forest plots showing the mean difference maize (top) and bean (bottom) yields in response to lime treatments in Rwanda based on random effects models. Note that the dashed line around the RE model polygon (diamond) at the bottom of each plot indicates the 95% prediction interval for the combined outcomes."}
...
forest(rm0, slab = labs, header="Country, Season, Site, Crop", 
       xlab = "Maize yield difference (kg/ha)", digits = 0, addpred=TRUE)

# Beans
forest(rb0, slab = labs, header="Country, Season, Site, Crop", 
       xlab = "Bean yield difference (kg/ha)", digits = 0, addpred=TRUE)
```

We can also look at the effects of the average lime amounts (t/ha) that were applied at the different sites across the Rwanda trials with a mixed-effects model.
...
Below is an example of a bubble plot showing the observed yield differences of the individual sites plotted against a quantitative predictor i.e., in this case the average lime application rate yield effects that are grouped by `season`, `site` and `crop` type in Rwanda. The size of the points is proportional to the precision weights that individual sites received in the analysis, with larger points receiving more weight. Based on the mixed-effects meta-regression model above, the predicted average yield response as a function of the lime application rate is also shown in the plot, with the corresponding 95% confidence interval. 

```{r, fig.align = 'center', fig.show='hold', out.width="80%", fig.cap = "Figure 2: Lime application rate effects on mean crop yield differences in Rwanda based on a site-level mixed-effects model."}
