...
Many surveys, such as this one, have difficulties in obtaining representative samples of individual trials across national and subnational areas. However, with proper statistical adjustment, nonrepresentative surveys could be used to generate accurate yield estimates. We just write the code here for reference because the data here would not support such analyses. We will run some initial multilevel models using the [`lmer`]() package here on the main PICO variables for these studies. Unfortunately, we cannot build response models for individual trials because these were not identified (georeferenced) or tracked over time. 
...
m1 <- lmer(yield ~ factor(crop) + factor(lime) * factor(fert) + (1|cell/year), data = lime)
```
