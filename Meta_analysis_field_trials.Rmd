...
This is the calculation for the expected yield differences (`delta` = treated yield - base yield) for maize as compared to the fertilizer only treatment, which is the designated comparator intervention in this case.
...
kend$delta <- kend$yhat - kend$bhat
```
...
* The simple initial question posed: "By how much could maize and bean yields be increased with lime applications?", can be answered using the causal modeling approach that was used here. What we do not know is if these *"improvements"* warrant the associated lime application and environmental costs to farmers, in the most affected places, and over the long-term.
