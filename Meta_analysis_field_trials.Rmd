...
* Put all raw survey and experiment data on KoBoToolbox. This is easy to set up and implement under realistic field conditions for which it is designed using [ODK](https://opendatakit.org/). Just go to [KoBoToolbox](https://www.kobotoolbox.org/) ... set it up in Excel (or anything else) and deploy it for your surveys or experiments ... it's free and easy to use! Then georeference, time stamp, and photograph all individual field trials. This is done easily with virtually any smart-phone these days. 

* Record intercrops. East Africa is not California or Nebraska. Intercrops occur virtually everywhere. Start designing trials input trials that actually reflect those facts.

* Repeat soil acidity (pH & exchangeable acidity) measurements over time. You don't really need a spectrometer to do that though it might help. [pH probes](https://www.amazon.com/ph-probe/s?k=ph+probe) sell for <50U$.

* Systematically document crop varieties used in trials. This would help enormously in assessing genotype × environment interactions ([GE](https://en.wikipedia.org/wiki/Gene%E2%80%93environment_interaction)) that plant breeders need to assess, recommend, and develop crop varieties in different places. 
