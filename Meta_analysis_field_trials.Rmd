...
The simplest, shortest and fasted way of screening extreme value outliers from these data is to consider the yield percentiles by crop. We can initially examine this by using [quantile regression]() as follows in the next chunk and subsequently decide which (if any), of the extreme values to screen from further analyses.
...
The other convenient is that you could also other PICO factors in the respective regressions for more comprehensive outlier explorations, but we shall leave to you to try those.
