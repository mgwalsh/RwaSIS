...
```{r, results = 'hide'}
# Package names
packages <- c("osfr", "dplyr", "quantreg", "DT", "metafor")
...
We can plot the `delta` values out to obtain a feel for the distribution of the aglime lime yield comparisons for the `r length(unique(rwa$trial_id))` OAF aglime trials that we consider to be eligible under our PICO definition for Rwanda.
...
plot(ecdf(rwa$delta/1000), main="", xlab="Aglime yield difference (t/ha)", ylab="ECDF", xlim=c(-6,6),
