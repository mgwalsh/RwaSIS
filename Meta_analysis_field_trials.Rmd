...
```{r}
# First remove the zeros ... they are not useful
...
The table above shows that ~5% of the crop yield measurements at its distributional (low and high) tail ends were flagged as potential extreme values ... which is what we were going for in this example. You can always adjust the percentile values in the `out.rq` regressions above. Another convenient thing about this approach is that you could also use additional PICO and/or other factors in those regressions for more comprehensive extreme value explorations. We shall leave those for you to test.

# Data modeling

Many surveys, such as this one, face difficulties in obtaining representative samples of individual trials across national and subnational areas. However, with proper statistical adjustment, many nonrepresentative surveys can be used to generate accurate yield estimates.

We will run some initial multilevel models using the [`lmer`]() package here on the main PICO variables for these studies. Unfortunately, we cannot build response models for individual trials because these were not identified (georeferenced) or tracked over time in the data that are available to us currently. 

```{r}
# We will use the outlier removed data here
lime <- lime[which(lime$out == 'n'), ]

m0 <- lmer(yield ~ factor(crop) + factor(lime) * factor(fert) + (1|study), data = lime)
m1 <- lmer(yield ~ factor(crop) + factor(lime) * factor(fert) + (1|year/cell), data = lime)
```
...
* East Africa is certainly not Nebraska or Brazil. Intercropping occurs virtually everywhere in Rwanda's smallholder production systems and elsewhere in East Africa. Our recommendation is to start conducting your agricultural input trials in ways that actually reflect those facts rather than on the basis of monocropping practices.
