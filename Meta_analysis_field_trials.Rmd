...
This is the calculation for the expected yield differences (`delta` = treated yield - base yield) for maize as compared to the fertilizer only treatment (the control in this case).
...
Note that for both maize and bean crops, yield increases due to lime application are expected to be quite modest in Rwanda. To illustrate this graphically, we can also plot the population-level cumulative distributions of the expected yields (`yhat`) in Rwanda with: 
...
# Main meta-analysis conclusion and takeaways
...
* In the trials presented, the application of lime showed modest positive intervention effects on crop yields that were observed over 11 cropping seasons vis-à-vis their respective control yields. The fairly low lime application rates in these trials may not have been sufficient to overcome the exchangeable (reserve) acidity constraints nor to effectively alter soil pH.

* As might be expected, space-time-crop interactions, i.e., the `season/site` varying effects are likely to dominate production function estimates and inference. However, these warrant more systematic investigations. One initial way of doing that would be to provide the georeference of each individual trial. A reanalysis of the data that are presented here, could subsequently be done quickly. 

* The simple initial question posed: "By how much could maize and bean yields be increased with lime applications?", can be answered using the causal modeling approach that was used. What we do not know is if these local improvements warrant the associated lime application and environmental costs to farmers, in the most affected places, over the long-term.
...
* Should you be interested in more than unreplicated case studies, ensure that individual experimental designs can actually be harmonized for more efficient and reliable learning. This bears careful thinking about the relevant PICO factors prior to initiating any additional liming projects and trials. 

* Once those factors have been thought through, then generate more efficient agronomic response trial designs. Conducting field trials is a quite expensive operational undertaking. The quality of the recorded data is also an important consideration, and sometimes less is more this context. We encourage you to explore leaner, more optimized (e.g., d-optimized, latin-hypercube, or other) trial designs than the ones that were used to generate the data that was used here. This would save money, improve data quality, and might also improve statistical inference and learning.
