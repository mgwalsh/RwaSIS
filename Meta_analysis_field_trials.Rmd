...
```{r}
sl <- aggregate(rwad$delta, list(rwad$site), FUN = length)
sm <- aggregate(rwad$delta, list(rwad$site), FUN = mean)
ss <- aggregate(rwad$delta, list(rwad$site), FUN = sd)
si <- cbind(sl, sm$x, ss$x)
colnames(si) <- c("site", "n", "delta", "sd")
```

```{r, fig.align = "center", site_index, fig.cap = "Expected overall crop yield differences (`delta`) to lime applications (in kg/ha) between trial sites in Rwanda. The thick parts of the lines represent 1 standard deviation from the mean difference values; the thin parts of lines represent 2 standard deviations around the mean (i.e., the 95% confidence interval)."}

par(pty="s")
coefplot(si$delta, si$sd, varnames = si$site, xlim = c(-600, 600), cex.pts = 2, cex.var = 0.9,
         CI = 2, main="")
```

Notice that the 95% CIs for the expected site-level lime response effects for the Rwanda trials are wide, and that all but one site (`nyakarera`) include 0 delta. This indicates that even at a site-level the expected yield gains from lime applications were also modest on average in these trials.
