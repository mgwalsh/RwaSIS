...
We can also plot the model fit to the trial data with the following chunk. This demonstrates that a fairly simple PICO multilevel model can explain a substantial amount of the variation in a fairly complex dataset. However, also note that this is not a prediction of what out-of-sample trials might look like.

```{r}
lime$m1 <- exp(fitted(m1, lime))

# Generate fitted vs observed plot


```

Other multilevel model specifications and analyses (e.g., see [Multilevel regression case studies](https://bookdown.org/jl5522/MRP-case-studies/)) in this context are certainly possible e.g., by also including variables such as crop varieties and genotype × environment interactions ([GE](https://en.wikipedia.org/wiki/Gene%E2%80%93environment_interaction)), seasonality, spot vs broadcast application, etc. But, the data would need to be collected and assembled to better reflect their specific PICO differences and very importantly the spatial variation in crop yields for these more contingent factors to make any measurable differences in the yield outcome variable that is considered here. 
