...
However, because of the lack of georeferencing of the individual trials it is not possible to tell which trials dropped-out during the second year in each study. Also, new trials may have come into the studies during the follow-up period that were not recorded as such ... but we have no way of telling. This may have affected the somewhat counter-intuitive crop yield differences that were recorded by time-period in the 2 studies.

```{r}
out <- aggregate(lime$yield, list(lime$year, lime$crop), FUN = quantile, probs = c(0.025, 0.975))
colnames(out) <- c("year", "crop", "Q")
out
```

The simplest way of screening the outliers from these data is to consider the crop yield quantiles by the main fixed PICO factors. We can initially examine this by using [quantile regression]() as follows in the next chunk and subsequently decide which values to screen from further analyses.
