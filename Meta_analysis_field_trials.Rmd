---
title: Meta-analysis of agricultual lime application effects on crop yields from field trial data
...
Many surveys, such as this one, will have difficulties in obtaining representative samples of individual trials across the relevant geographical areas of interest. However, with proper statistical adjustment of the relevant variance components (random effects), non-representative surveys can be used to generate and monitor crop yields more accurately. A multilevel model is a hierarchical linear regression model, which assumes that the data being analyzed are drawn from a hierarchy of different populations whose differences relate to that hierarchy. 

While we cannot build response models for individual trials because these were not identified (georeferenced) or tracked over time in these studies, we will run some initial multilevel models that are grouped by `cell` and `trial_age` using the [`lme4`](https://cran.r-project.org/web//packages/lme4/lme4.pdf) package on the main PICO variables for the 2 studies. The following chunk provides some examples, which could be amended once all of the individual trial PICO have been specified.
...
m1 <- lmer(log(yield) ~ factor(trial_age) + factor(crop) * factor(lime) * factor(fert) + (1|study:cell) +
          (1|cell:trial_age) + (1|cell:crop), data = lime)

m2 <- lmer(log(yield) ~ factor(trial_age) + factor(crop) * factor(lime_amt) * factor(fert) + (1|study:cell) +
          (1|cell:trial_age) + (1|cell:crop), data = lime)

anova(m0, m1, m2)
```

Model `m1` is clearly preferable to the overly simple `m0` model based on the respective AIC, BIC and logLik comparisons in the between model `anova`. The `m2` model, provides a slightly better fit to the data than `m1`. However, the choice between the `m1` and `m2` models appears to be a matter of preference as they differ only in terms of the number of classes that were used to describe the lime treatment levels. The `m2` also has a singularity problem in the REML formulation based on these data. Because of the overall meta-analysis study design and for reasons of parsimony, we prefer model `m1`. The estimated [production function](https://en.wikipedia.org/wiki/Production_function) coefficients for the `m1` model are shown directly below.   

```{r}
# Write the fits
lime$m1 <- exp(fitted(m1, lime))
yq <- rq(yield~m1, tau=c(0.025,0.5,0.975), data = lime) ## quantile regression fit
display(m1)
```

We can also plot the model fit to the trial data with the following chunk. This demonstrates that a fairly simple PICO-based multilevel model can explain a substantial amount of the variation in a fairly complex dataset. However, also note that this plot is not intended as a prediction of what out-of-sample trials might look like ... it just shows an unvalidated fit to the data.

```{r, fig.align = "center", fig.cap = "Quantile regression plot of modeled vs observed crop yields in Rwanda. The blue lines are the 2.5% and 97.5% quantile regression estimates. The red line is the robust median fit."}

par(pty="s", mar=c(4,4,1,1))
plot(yield~m1, xlab="Production function fit", ylab="Observed yield (kg / ha)", cex.lab=1.3, 
     xlim=c(0,10000), ylim=c(0,10000), lime)

curve(yq$coefficients[2]*x+yq$coefficients[1], add=T, from=0, to=10000, col="blue", lwd=2)
curve(yq$coefficients[4]*x+yq$coefficients[3], add=T, from=0, to=10000, col="red", lwd=2)
curve(yq$coefficients[6]*x+yq$coefficients[5], add=T, from=0, to=10000, col="blue", lwd=2)
abline(c(0,1), col="grey", lwd=1)
```

Other multilevel model specifications and analyses (e.g., see [Multilevel regression case studies](https://bookdown.org/jl5522/MRP-case-studies/)) in this context are certainly possible e.g., by also including variables such as crop varieties and genotype × environment interactions ([GE](https://en.wikipedia.org/wiki/Gene%E2%80%93environment_interaction)), seasonality, spot vs broadcast application, etc. But, the data would need to be collected and assembled to better reflect their specific PICO-differences and very importantly also the spatial variation for the more contingent factors in yield outcomes that were considered here. 

# Main case-study conclusion and takeaways

*Based on the two recent crop yield studies that were analyzed here, we currently would not be able to justify nor recommend investments in wide-spread aglime applications in the maize and bean cropping systems of Rwanda. The data, though extensive, practical and very useful were insufficient to warrant recommending that these types of interventions should be applied at a either a population or local scale for maize or bean crops.*

The main takeaways of this notebook are the following:

* In the two studies presented the application of lime showed only modest positive effects on either maize or bean crop yields that were observed over a two year period vis-à-vis their respective control yields. The fairly low lime application rates in these trials may not have been sufficient to overcome the exchangeable (reserve) acidity constraints nor to effectively alter soil pH over the 2-year duration of the trials.

* The simple initial question posed: "By how much could maize and bean yields be increased with lime applications?", could not be satisfactorily answered in this notebook. From the presented data we can only infer that there were modest yield improvements in some of the surveyed administrative units. What we do not know is if these local improvements warrant the associated lime application costs to farmers, in those places, over the long-term.
...
* Repeat soil acidity (pH & exchangeable acidity) measurements over time. You don't really need a spectrometer to do that, though it might help ... also with other related soil properties. Good [pH probes](https://www.amazon.com/ph-probe/s?k=ph+probe) sell for ~50 U$ on Amazon.
