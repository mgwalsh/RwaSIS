...
The subsequent sections demonstrate a relatively generic prediction workflows that use ensembles of different machine learning (ML) algorithms (see e.g. the various review articles at: [Emsemble models](https://www.sciencedirect.com/topics/computer-science/ensemble-modeling)) for predicting soil and cropping system condition data using remote sensing and GIS. While the *"legacy data"* used in this example were not collected on a replicable geostatistical sampling frame, they provide an inital indication of where specific soil problems or nutrient deficiencies may be prevalent in Rwanda; thus, warranting potentially different management inteventions. As new data become available over the course of the project, we will update data sources and analysis workflows. 

# Data setup

For each predictive soil map, you'll intially need a data setup. If you are using [R]() you'd need to use the packages indicated in the chunk directly below.
