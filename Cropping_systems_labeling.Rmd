...
* We used association rule (AR) mining workflows to explore dominant staple food crop combinations that are both prevalent and potentially informative; that is, ARs that exhibit adequate support, confidence and lift to consider them to be of generalizable interest for identifying staple crop diversification pathways in both countries.

* Notably, of the 2^20^-1 potential staple crop combinations that might occur at random, we have identified `r nrow(RW_rules_rhs)` ARs in Rwanda and `r nrow(TZ_rules_rhs)` ARs in Tanzania, which are both prevalent and potentially interesting for agricultural and human dietary diversity management. 

* From the exploratory analyses it is quite clear that ARs in both national surveys depend strongly on the proximity to buildings. Specifically, there is a subset of 10 indicator staple crops including cattle, goats, sheep, poultry, maize, sorghum, beans, sweet potato and banana that are strongly associated with the occurrence of buildings (see Figure 9 and its AR table).
