...
The ROC curve can also be used to threshold the predictions to create a cropland mask map for Rwanda. Pixels above (or below) the threshold are classified as having cropland present (or absent). This is useful for additional accuracy checks. Additionally, this step provides a map that is useful for planning of subsequent / additional ground sampling campaigns that involve croplands as the main ROI (e.g., for soil mapping and/or various management response trials). The next chunk sets the relevant threshold and provides a map of the cropland mask for Rwanda.
