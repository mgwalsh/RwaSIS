...
While the national-level estimate of cropland proportion (`m0`) is not affected much, its precision as measured by the residual deviance comparison between the two models (`m0` vs `m1`) is. We could also try to fit this by including the individual base model predictions to derrive predictions similar to those of the stacked (`st`) model. This could introduce an alternative SAE weighting scheme for further consideration and testing. The main point here is to consider the main stacking options for the cropland area predictions.
...
The slightly lower deviance of the `m2` model for area predictions indicates that it might be preferable. Note however that the difference is small enough and has not been validated. So, I would still go with the `m1` prediction in this particular case, based on the [parsimony principle](https://en.wikipedia.org/wiki/Occam%27s_razor).
...
* This notebook produces highly accurate (>95%) spatial predictions of the observed occurence (presence / absence) and the associated *small cropland areas* in Rwanda based on a completely reproducible, standard ensemble machine learning workflow (e.g., see [Rocca (2019)](https://towardsdatascience.com/ensemble-methods-bagging-boosting-and-stacking-c9214a10a205)).
...
* The model outputs can be used to improve the precision of small area estimates but potentially also for cropland monitoring workflows.

* Model outputs can also be used for planning and implementing targeted ground survey campaigns (e.g., soil surveys) that focus on cropland ROIs ... currently ongoing in Rwanda and other African countries.

I'll update the introductory examples in this notebook as additional ground data for the occurrence of e.g., specific crops, crop types and soil / site surveys become available for Rwanda over the coming year via the ongoing [RwaSIS project](https://minagri.prod.risa.rw/updates/news-details/rwanda-launches-soil-information-service-project-to-improve-agriculture-productivity).
