...
The slightly lower deviance of the `m2` model for area predictions indicates that it maybe preferable. Note however that the difference is small enough and is not validated. So, I would still go with the `m1` estimate in this particular case based on the local data and the [parsimony principle](https://en.wikipedia.org/wiki/Occam%27s_razor).
...
* Model outputs can also be used for planning and implementing targeted ground survey campaigns that focus on cropland ROIs ... ongoing in Rwanda and other African countries currently.

I'll update the introductory examples in this notebook as additional ground data for the occurrence of e.g., specific crops, crop types and site conditions become available for Rwanda over the coming year via the ongoing [RwaSIS project](https://minagri.prod.risa.rw/updates/news-details/rwanda-launches-soil-information-service-project-to-improve-agriculture-productivity).
