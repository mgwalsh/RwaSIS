...
# Number of tagged grid locations (out of 16 possibles) from quadrats with cropland
...
cp <- cbind(cp, ccount)
geos <- rbind(ca, cp)
geos <- geos[order(geos$time), ] ## sort back in original random sample order
```
...
[ROC analysis](https://en.wikipedia.org/wiki/Receiver_operating_characteristic) helps to select a potentially optimal model for predicting a binary outcome variable and to discard any suboptimal ones. I use it here to assess the classification performace of the stacked model (`st`) on the validation dataframe.
...
This notebook produces highly accurate spatial predictions of cropland occurence and area for Rwanda.
