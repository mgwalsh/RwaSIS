...
Note that I am using previously vetted cropland presence/absence (`CP`) as an example in this context. This produces probability maps of where in Rwanda croplands are likely to occur versus where they are likely to be absent. You can also substitute other GeoSurvey variables as labels and specify those with the `labs` variable in the chunk above. The additional variables are also included in the `RW_soil_data.csv` data file for you to try. 
