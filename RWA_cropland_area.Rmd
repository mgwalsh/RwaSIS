...
4. Model prediction results are subsequently placed back into the gridded feature stack for reuse and updating. This step is really useful for improving model performance over time with new data and/or for building ensembles at higher spatial resolution.
...
In order to monitor changing landscapes the first three steps should be repeated over time; i.e., to facilitate step 4. The end of this notebook provides some suggestions on as to how to do just that. The next chunk initially scrubs some of the extraneous objects in memory, sets-up labels and features, and creates a randomized partition between the training and validation dataframes.
