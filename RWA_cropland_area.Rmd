...
Note that I am using previously vetted cropland presence/absence (`CP`) as an example in this example. This produces probability maps of where in Rwanda croplands are likely to occur versus where they are unlikely to be present. You can also substitute other GeoSurvey variables as labels and specify those with the `labs` variable in the chunk above. While these additional variables are included in the `RW_soil_data.csv` data file, but I shall leave those for you to explore. Presented next are some starter models, which I would consider to be contrasting both in terms of the gridded features that are used, as well as the MLAs that are used in fitting the data to those.
...
gf_cpv <- gs_cal[,39:41] ## these are the DX, DY & DXY raster features
...
gf_cpv <- gs_cal[,25:38] ## these are the anthropic distance-to feature variables
