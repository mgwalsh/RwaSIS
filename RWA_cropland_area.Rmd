...
This assembles the GeoSurvey observations from *n* = 23,776, 6.25 ha ($$250 * 250$$ m) [quadrats](https://en.wikipedia.org/wiki/Quadrat). It also calculates building counts as well as cropland grid counts from the associated point location geojson fields (`bloc` & `cgrid`) in this GeoSurvey. I shall use the resulting cropland grid count variable (`ccount`) for small area estimation later on in the notebook.
