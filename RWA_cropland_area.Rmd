...
This next chunk assembles the various GeoSurvey observations for Rwanda and overlays these with the various raster and shape file data. It also calculates building count (`bcount`) as well as cropland grid count data (`cgrid`) for the associated geojson fields in this GeoSurvey. I will use the resulting `ccount` variable for small cropland area estimation later on in the notebook. 
...
The figure directly below attempts to clarify the GeoSurvey data based on 4 recorded 6.25 ha (250x250 m) quadrats from Rwanda. The top left portion of the figure shows all tagged buildings, in quadrats where buildings are present, the top right of the figure shows an example of a cropland grid count where cropland is present, the bottom left is a quadrat with dense woody vegetation cover (> 60%), the bottom right quadrat is an example of grassland without any buildings, cropland and dense woody vegetation.
