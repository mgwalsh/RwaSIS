...
While the national-level estimate of cropland proportion (`m0`) is not affected much, its precision as measured by the residual deviance comparison (precision) between the two models (`m0` vs `m1`) is. We could also try to fit this by including the individual base model predictions to derrive stacked predictions similar to those of the stacked (`st`) model.

```{r}
# Post-stratified spatial model with GeoSurvey cropland occurrence predictions
m2 <- glm(cbind(ccount, 16-ccount) ~ gm+cp+gl+rf+xt+nn, family = binomial, gsout)
m2.pred <- predict(gspreds, m1, type = "response") ## maps pixel-level predictions
