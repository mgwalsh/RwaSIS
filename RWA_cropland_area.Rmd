...
While the national-level estimate of cropland proportion (`m0`) is not affected much, its precision as measured by the residual deviance comparison (precision) between the two models (`m0` vs `m1`) is. We could also try to fit this by including the individual base model predictions to derrive predictions similar to those of the stacked (`st`) model. This could introduce an alternative SAE weighting scheme for further consideration. The main point here is to consider the main stacking options for cropland area predictions.
...
* This notebook produces highly accurate (>95%) spatial predictions of the observed occurence (presence / absence) and the associated *small cropland areas* in Rwanda based on a completely reproducible, standard ensemble machine learning workflow.
...
I'll update the introductory examples in this notebook as additional ground data for the occurrence of specific crops and crop types become available for Rwanda over the coming year via the ongoing [RwaSIS project](https://minagri.prod.risa.rw/updates/news-details/rwanda-launches-soil-information-service-project-to-improve-agriculture-productivity).
