...
The above chunk calculates the overall cropland area of Rwanda at ~68% of its total land area of ~2.37 Mha. The standard 95% confidence interval of this estimate is ± 1%. The next chunk calculates post-stratified estimates using the GeoSurvey cropland occurrence predictions. It also produces map of the expected within pixel cropland proportions and areas. While the national-level estimate of cropland proportion is not affected much, its precision is. 
...
```{r}
# Small area random-effect estimates by district & sector
m2 <- glmer(cbind(ccount, 16-ccount) ~ st + (1|district) + (1|sector), family = binomial, gsout)
summary(m2)
```

# Conclusions

* This notebook produces highly accurate (>95%) spatial predictions of the observed occurence (presence / absence) of croplands in Rwanda based on a completely reproducible ensemble machine learning approach.
* The approach can be flexibly extended to map other land cover variables (e.g., the occurrence of buildings, dense woody vegetation cover, built-up infrastructure and crop type distributions, among others). It can also be rapidly extended to cover new geographies and ROIs.
* The model outputs can be used to improve the precision of small area estimates and potentially also for cropland monitoring workflows. I'll update the examples in this notebook as additional ground data become available for Rwanda over the coming year via the currently ongoing [RwaSIS project](https://minagri.prod.risa.rw/updates/news-details/rwanda-launches-soil-information-service-project-to-improve-agriculture-productivity). 
