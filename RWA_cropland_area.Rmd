...
author: M.G. Walsh, J. Chen and W. Wu
...
This next chunk assembles the various GeoSurvey observations for Rwanda from (*n* = 23,776) 6.25 ha quadrats. It also calculates building count (`bcount`) as well as cropland grid count data (`cgrid`) in the associated geojson fields in this GeoSurvey. I shall use the resulting quadrat `ccount` variable for small cropland area estimation later on in the notebook.
...
The following chunk then writes out the dataframe `RW_GS_data.csv` into your `./Results` directory if you'd like to process those outputs in software other than R. It also generates a location map of where those 23k+ GeoSurvey observations were obtained. 

```{r}
# Write data frame
write.csv(geos, "./RW_GS20/Results/RW_soil_data.csv", row.names = F)
