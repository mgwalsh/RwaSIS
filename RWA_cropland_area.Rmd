...
2. Calibration and model stacking, which involve calibrating several potentially contrasting MLAs with cross-validation. I shall use a combination of generalized linear, bagging, boosting and neural network base models here (see e.g., [Rocca (2019)](https://towardsdatascience.com/ensemble-methods-bagging-boosting-and-stacking-c9214a10a205) for a more thorough explanation of those. 
...
```{r}
# Post-stratified spatial model with GeoSurvey cropland occurrence predictions
...
While the national-level estimate of cropland proportion (`m0`) is not affected much, its precision as measured by the residual deviance comparison (precision) between the two models (`m0` vs `m1`) is. The next chunk calculates small area estimates for the 30 national *districts* and 417 *sectors located in districts* that are reporting cropland area information to the Ministry of Agriculture ([MINAGRI](https://www.minagri.gov.rw/)) in Rwanda. Again, the inclusion of the country-wide GeoSurvey cropland occurrence predictions in the respective area models provide potentially useful information for sub-national planning purposes.

```{r}
# Small area hierarchical random-effect estimates by 417 sectors in 30 districts
m2 <- glmer(cbind(ccount, 16-ccount) ~ 1 + (1|district/sector), family = binomial, gsout)
m3 <- glmer(cbind(ccount, 16-ccount) ~ st + (1|district/sector), family = binomial, gsout)
anova(m2, m3) ## anova comparison
```

# Takeaways

* This notebook produces highly accurate (>95%) spatial predictions of the observed occurence (presence / absence) and the associated *small cropland areas* in Rwanda based on a completely reproducible ensemble machine learning workflow.

* The workflow can be flexibly extended to map other land cover variables (e.g., the occurrence of buildings, dense woody vegetation cover, built-up infrastructure and crop type distributions, among others). It can also be rapidly extended to cover new geographies and ROIs.
...
I'll update the introductory examples in this notebook as additional ground data become available for Rwanda over the coming year via the ongoing [RwaSIS project](https://minagri.prod.risa.rw/updates/news-details/rwanda-launches-soil-information-service-project-to-improve-agriculture-productivity).
